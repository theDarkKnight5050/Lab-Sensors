 ...
 * 
 * @webref serial
 * @brief Class for sending and receiving data using the serial communication
 *        protocol.
...
public class Serial implements SerialPortEventListener {
	...
	/**
	 * @param parity
	 *            'N' for none, 'E' for even, 'O' for odd, 'M' for mark, 'S' for
	 *            space ('N' is the default)
	 * @param dataBits
	 *            8 is the default
	 * @param stopBits
	...
	public static String[] getProperties(String portName) {
		return SerialPortList.getPortNames(portName);
	}

	/**
	 * @generate Serial_last.xml
	 *           <h3>Advanced</h3> Same as read() but returns the very last
	 *           value received and clears the buffer. Useful when you just want
	 *           the most recent value sent over the port.
	 * @webref serial:serial
	 * @usage web_application
	 */
	public int last() {
	...
	/**
	 * <h3>Advanced</h3> Return a byte array of anything that's in the serial
	 * buffer up to the specified maximum number of bytes. Not particularly
	 * memory/speed efficient, because it creates a byte array on each read, but
	 * it's easier to use than readBytes(byte b[]) (see below).
	 *
	 * @param max
	 ...
	 *           <h3>Advanced</h3> If you want to move Unicode data, you can
	 *           first convert the String to a byte stream in the representation
	 *           of your choice (i.e. UTF8 or two-byte Unicode data), and send
	 *           it as a byte array.
	 *
	 * @webref serial:serial
	 * @usage web_application
	 ...
	 *           <h3>Advanced</h3> Write a String to the output. Note that this
	 *           doesn't account for Unicode (two bytes per char), nor will it
	 *           send UTF8 characters.. It assumes that you mean to send a byte
	 *           buffer (most often the case for networking and serial i/o) and
	 *           will only use the bottom 8 bits of each char in the string.
	 *           (Meaning that internally it uses String.getBytes)
	 *
	 *           If you want to move Unicode data, you can first convert the
	 *           String to a byte stream in the representation of your choice
	 *           (i.e. UTF8 or two-byte Unicode data), and send it as a byte
	 *           array.
	 *
	 * @webref serial:serial
	 * @usage web_application
